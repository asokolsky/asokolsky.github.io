...
1. Checks the tags of the object affected, if the object is not IaC, the change is ignored.
1. Calculates the timeframe for the lookup of the corresponding events in the CLoudTrail - expected to be within 5 min.
1. Performs the CloudTrail lookup - we are interested in the user identity first and foremost.
1. Finally, lambda publishes the message to Slack outlining who changed which IaC object and when.
